...
assert False,'this is deprecated in fabor of gpnn_gradcam_multi?'
#%%
import os
import numpy as np
...
if os.environ.get('USE_GPNN_FAST',False) == '1':
    ...
    assert False
elif os.environ.get('USE_GPNN_TRANSPARENCY',False) == '1':
    print(colorful.chartreuse2('using gpnn transparency'))
    from model.my_gpnn_try_transparency  import extract_patches,combine_patches    
    from model.my_gpnn_try_transparency import gpnn
    assert False    
elif os.environ.get('USE_GPNN_MAP_AUGMENTED',False) == '1':
    print(colorful.chartreuse2('using gpnn map augmented'))
    from model.my_gpnn_map_augmented  import extract_patches,combine_patches    
    from model.my_gpnn_map_augmented import gpnn    
    # assert False    
elif os.environ.get('USE_GPNN_TRY_DIFF',False) == '1':
    print(colorful.chartreuse2('using gpnn map augmented'))
    from model.my_gpnn_try_diff  import extract_patches,combine_patches    
    from model.my_gpnn_try_diff import gpnn    
    # assert False        
else:
    ...
    for i in range(n_super_iters):
        ...
        if i > 0:
            if True:
                ...
                if running_saliency_dict['avg_saliency'].shape[-2:] != config_i['input_img'].shape[-2:]:
                    import ipdb;ipdb.set_trace()
        ...
        if i == 0:
            assert cam0_.ndim == 2
            dutils.cam = cam0_[None,None,...]
        else:
            
            dutils.cam = torch.tensor(running_saliency_dict['avg_saliency'],device=device)
            # import ipdb;ipdb.set_trace()
...
if __name__ == '__main__':    
    if True:
        ...
        assert False,'update config, doesnt have index type, copy from pascal_run...'
        config = {
