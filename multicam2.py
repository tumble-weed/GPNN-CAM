...
from relevance_cam_modules.vgg import vgg16, vgg16_bn, vgg19, vgg19_bn
from relevance_cam_modules.resnet import resnet50, resnet101, resnet18
...
'''
# args.models = 'vgg16'
# args.target_layer = 'layer2'
# args.target_class = None
# define data loader

###########################################################################################################################
#model_arch = args.models
def init_model(model_arch,target_layer,device='cpu'):
    if model_arch == 'vgg16':
        model = vgg16_bn(pretrained=True).to(device).eval()  #####
        target_layer = model.features[int(target_layer)]
    elif model_arch == 'vgg19':
        model = vgg19_bn(pretrained=True).to(device).eval()  #####
        target_layer = model.features[int(target_layer)]
    ...
    return Score_CAM_class,model,value
#====================================================
...
# img is a np array here
from gradcam import normalize_tensor 
def relevance_cam(img:torch.Tensor,model_arch,target_layer,target_class):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    Score_CAM_class,model,value = init_model(model_arch,target_layer,device=device)
    in_tensor = normalize_tensor(img).to(device)
    R_CAM, output = model(in_tensor, target_layer, [target_class])

    if target_class == None:
        maxindex = np.argmax(output.data.cpu().numpy())
    else:
        maxindex = target_class
    ...
    R_CAM = tensor2image(R_CAM,shape=in_tensor.shape[-2:])
