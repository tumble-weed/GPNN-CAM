...
class Perturbation:
    ...
    def apply(self, mask):
        ...
        if False:
            ...
            # assert False,'hack for ste'
            print('hack for ste')
            # import pdb;pdb.set_trace()
            w = mask.reshape(n, 1, *mask.shape[1:])
            y0 = torch.gather(y, 1, torch.zeros_like(k))
            y1 = torch.gather(y, 1, torch.clamp(
                (self.num_levels-1) * torch.ones_like(k), 
...
def get_masked_input(model,
    ...
    if isinstance(perturbation,str):
        global perturbation_obj
        if 'perturbation' not in globals():
            perturbation_obj = Perturbation(
                input,
                num_levels=num_levels,
                type=perturbation
            ).to(device)
        perturbation = perturbation_obj
    ...
    # Apply the mask.
    if variant == DELETE_VARIANT:
        x = perturbation.apply(1 - mask)
    elif variant == PRESERVE_VARIANT:
        x = perturbation.apply(mask)
    elif variant == DUAL_VARIANT:
        x = torch.cat((
            perturbation.apply(mask),
            perturbation.apply(1 - mask),
        ), dim=0)
    else:
        assert False
