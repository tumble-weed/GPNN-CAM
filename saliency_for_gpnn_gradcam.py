assert False,'this file is deprecated in favor of saliency_for_gpnn_gradcam_multi.py'
...
def normalize_tensor(t,vgg_mean=[0.485, 0.456, 0.406],
    ...
    return out
'''
def get_cams(augmentations,imagenet_target,method=None,gradcam_scale_cams = False,cnn=None):
    ...
    if method in ['gradcam','gradcampp']:
        if GRADCAM_IMPL == 'pytorch_grad_cam': 
            ...
            cams,scores,probs = gradcam.gradcam(augmentations,target=imagenet_target,model_type='imagenet',scale=gradcam_scale_cams,cnn=cnn)
            ...
            # import ipdb;ipdb.set_trace()
            _,cams,cams_pp,scores,probs = CAM_CLASS(normalized_augmentations, class_idx=imagenet_target)
            if method == 'gradcampp':
                # import ipdb;ipdb.set_trace()
                cams = cams_pp
            if cams.ndim == 2:
            ...
            print('TODO: how about using gradcampp instead of gradcam?')
            #==================================================
        ...
        assert augmentations.min() >= 0
        assert augmentations.max() <= 1
        normalized_augmentations = normalize_tensor(augmentations)
