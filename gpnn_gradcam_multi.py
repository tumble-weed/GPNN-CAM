...
import colorful
from saliency_for_gpnn_gradcam_multi import get_saliency,permute_using_I,get_cams
...
def augmentation_attribution(config=None,cnn=None,dataset='imagenet',running_saliency_dict={}):
    ...
    # import pdb;pdb.set_trace()
    saliency_dicts = get_saliency(augmentations,aggregation_results,(config['patch_size'],config['patch_size']),imagenet_target,saliency_method=config['base_method'],cnn=cnn,dataset=dataset,running_saliency=[running_saliency_dict])
    saliency_dict = saliency_dicts[0]
    return augmentations,aggregation_results,saliency_dict,model
def main(config=None,cnn=None,save_results = False,more_returns={},dataset='imagenet'):
    ...
    if True and 'original image':
        ...
        for k in out0:
            cam0 = out0[k][0]
            cam0 = torch.tensor(cam0).unsqueeze(1).to(device)
            more_returns[k] = { 'cam0' : tensor_to_numpy(cam0)[0,0]}
    ...
    for i in range(n_super_iters):
        ...
        if  i ==0:
            if False:
                assert False,'not compatible with multi cams'
                import ipdb;ipdb.set_trace()
                cam0_ = cam0[0,0]
        ...
        if os.environ.get('USE_GPNN_TIMING',False) == "1":
            ...
            import ipdb;ipdb.set_trace()
        augmentations,aggregation_results,saliency_dict,model =   augmentation_attribution(config=config_i,cnn=cnn,dataset=dataset,running_saliency_dict=running_saliency_dict)
        ...
        if save_results:
            assert False,'save_results not corrected for multi cam returns'
            # save
            # augmentations
