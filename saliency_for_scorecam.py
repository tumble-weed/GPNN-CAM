...
def normalize_tensor(t,vgg_mean=[0.485, 0.456, 0.406],
    ...
    return out
'''
def get_cams(augmentations,imagenet_target,method=None,gradcam_scale_cams = False,cnn=None,arg_target_layer=None):
    ...
    if method in ['gradcam','gradcampp']:
        ...
        # from gradcam2.libre_gradcam import my_ScoreCAM_multi
        ...
        CAM_CLASS = my_GradCAM_multi(cnn,arg_target_layer=arg_target_layer)
        #==================================================
        ...
        if False:
            ...
            if not any([augmentations.min() < 0, augmentations.max() > 1]):
                print(colorful.yellow_on_red(f'{method}:is the augmentation normalized?'))
            normalized_augmentations = (augmentations)
        dutils.cipdb('DBG_PASCAL_NORMALIZATION')
        # score_saliency_map, score_weight
        # import ipdb;ipdb.set_trace()
        # a, grad_cam, grad_cam_pp,scores,probs
        if method == 'gradcam':
            activations,cams,_,scores,probs = CAM_CLASS(normalized_augmentations, class_idx=imagenet_target)
            scores = None;probs = None
            if cams.ndim == 2:
                cams = cams[None,...]
        elif method == 'gradcampp':
            activations,_,cams,scores,probs = CAM_CLASS(normalized_augmentations, class_idx=imagenet_target)
            # import ipdb;ipdb.set_trace()
            scores = None;probs = None
            if cams.ndim == 2:
                cams = cams[None,...]            
 
    elif 'trivialcam' in method:
        # import ipdb;ipdb.set_trace()
        # base_method = method[len('trivialcam_'):]
        from gradcam2.libre_gradcam import my_GradCAM_multi
        # from gradcam2.libre_gradcam import my_ScoreCAM_multi
        #from gradcam2.Multi_cam import multi_CAM 
        # from gradcam2.relevance_cam_utils.LRP_util import GradCAM_multi
        ...
        # HFLIP   
        # import ipdb;ipdb.set_trace()
        HFLIP = 'hflip' in method
        NOISE = 'noise' in method
        n_noisy = 100 if not HFLIP else 50

        flip_indicator = torch.zeros(normalized_augmentations.shape[0],device=device).bool()
        if HFLIP:
            naug_prev = normalized_augmentations.shape[0]
            normalized_augmentations = torch.cat([normalized_augmentations,normalized_augmentations.flip(2)],dim=0)
            new_flip_indicator = torch.zeros(normalized_augmentations.shape[0])
            new_flip_indicator[naug_prev:] = 1
            flip_indicator = new_flip_indicator
        if NOISE:
            # NOISE
            normalized_augmentations = normalized_augmentations.unsqueeze(1) + 0.1*torch.randn(1,n_noisy,*normalized_augmentations.shape[1:],device=device)
            flip_indicator = flip_indicator.unsqueeze(1) * torch.ones((1,n_noisy),device=device)
            flip_indicator = flip_indicator.bool()
            normalized_augmentations = normalized_augmentations.flatten(start_dim=0,end_dim=1)
            flip_indicator = flip_indicator.flatten()
            assert flip_indicator.shape[0] == normalized_augmentations.shape[0]
            #============================================================================        
        if 'gradcampp' in method:
            base_method = 'gradcampp'
        else:
            assert 'gradcam' in method
            base_method = 'gradcam'
        if base_method == 'gradcam':
            activations,cams,_,scores,probs = CAM_CLASS(normalized_augmentations, class_idx=imagenet_target)
            scores = None;probs = None
            if cams.ndim == 2:
                cams = cams[None,...]
        elif base_method == 'gradcampp':
            activations,_,cams,scores,probs = CAM_CLASS(normalized_augmentations, class_idx=imagenet_target)
            import ipdb;ipdb.set_trace()
            scores = None;probs = None
            if cams.ndim == 2:
        ...
        #==================================================        
        flip_indicator = flip_indicator.bool()
        cams = torch.tensor(cams).unsqueeze(1).to(device)
        cams[flip_indicator] = cams[flip_indicator].flip(2)
        cams = cams.mean(dim=0,keepdim=True)
        cams = cams[:,0]
        cams = tensor_to_numpy(cams)

    elif method == 'relevancecam':
        ...
        # score_saliency_map, score_weight
        # import ipdb;ipdb.set_trace()
        cams = CAM_CLASS(normalized_augmentations, class_idx=imagenet_target)
        scores = None;probs = None
        if cams.ndim == 2:
            cams = cams[None,...]
        # import ipdb;ipdb.set_trace()
    elif method == 'libracam':
        assert False,'not implemented'
        from gradcam2.libre_gradcam import my_RelevanceCAM
        #from gradcam2.Multi_cam import multi_CAM 
        # from gradcam2.relevance_cam_utils.LRP_util import GradCAM_multi
        ...
        if GRADCAM_IMPL == 'libre_cam':
            ...
            CAM_CLASS = my_ScoreCAM_multi(cnn)
            #==================================================
            # return a, grad_cam, grad_cam_pp,scores,probs
            if False:
                normalized_augmentations = normalize_tensor(augmentations)
            ...
            # score_saliency_map, score_weight
            # import ipdb;ipdb.set_trace()
            cams,score_weight = CAM_CLASS(normalized_augmentations, class_idx=imagenet_target)
            scores = None;probs = None
            if cams.ndim == 2:
                cams = cams[None,...]
            # import ipdb;ipdb.set_trace()
            #==================================================
            #grad_cam,grad_cam_pp,relevance_cam,score_cam = multi_CAM(test_model,cnn,augmentations)
        # import ipdb;ipdb.set_trace()
...
'''
def get_saliency(augmentations,aggregation_results,patch_size,target,saliency_method='gradcam',cnn=None):
    device = augmentations.device
    gradcam_scale_cams = False
    assert patch_size[0] == patch_size[1]
    cams,scores,probs = get_cams(augmentations,target,method=saliency_method,gradcam_scale_cams=gradcam_scale_cams,cnn=cnn)
