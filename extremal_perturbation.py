...
from torchray_common import resize_saliency
from saliency_elp import permute_using_I
...
def extremal_perturbation(model,
    ...
    optimizer = optim.SGD([pmask],
                          ...
                          momentum=momentum,
                          dampening=momentum)
    hist = torch.zeros((len(areas)*input.shape[0], 2, 0))
    ...
    for t in range(max_iter):
        ...
        if patch_aggregation_info is not None:
            assert patch_size is not None
            
            mask_ = permute_using_I(mask_.tile(input.shape[0],1,1,1),
                                       patch_aggregation_info['I'],
                                       mask_.shape[-2:]+(1,),patch_size=patch_size,
                                       patch_aggregation = patch_aggregation_info['patch_aggregation'],
                                       W= patch_aggregation_info['weights'],
                                       stride=1)
        ...
        if debug_this_iter:
            ...
            for i, a in enumerate(areas):
                ...
                if variant == DUAL_VARIANT:
                    ...
                    imsc(x[i + len(areas)*input.shape[0]])
                plt.pause(0.001)
