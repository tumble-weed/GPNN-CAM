    ...
    for dataset in datasets:
        for base_methodname in base_methodnames:
            for modelname in modelnames:
                    dataset_stub = dataset+"-" if dataset == "pascal" else ""
                    ...
                    # import ipdb;ipdb.set_trace()
                    command = " ".join([f"{k}={v}" for k,v in env_flags.items()])
                    command=command + " " + f'CUDA_VISIBLE_DEVICES={device} python -m {saliency_file} --methodnames {base_methodname} --modelname {modelname} --skip {skip} --skip_incorrect {skip_incorrect} --dataset {"imagenet" if dataset == "" else dataset}'
