...
def embed_in_larger(t1u,output_shape,patch_size,stride=(1,1)):
    device = t1u.device
    h,w = output_shape
    ph,pw = patch_size
    out = np.inf *torch.ones((1,1,h+2*(ph//2),w + 2*(pw//2),ph,pw),device=device)
    out[:,:,2*(ph//2):-2*(ph//2):stride[0],2*(pw//2):-2*(pw//2):stride[1]] = t1u
    return out
def overlap_form(unfolded,output_shape,patch_size,sort_order=None):
    ...
    At = torch.meshgrid(torch.arange(ph//2,h+(ph//2),device=device),torch.arange(pw//2, w+(pw//2),device=device),indexing='ij')
    I2 = YY[None,None,:,:] + (At[0][:,:,None,None] - (ph//2))
    I3 = XX[None,None,:,:]  + (At[1][:,:,None,None] - (pw//2))
    I4 = ph -1 - (torch.zeros_like(At[0])[:,:,None,None] +  YY[None,None,:,:])
    I5 = pw -1 - (torch.zeros_like(At[1])[:,:,None,None] +  XX[None,None,:,:])
...
def get_median(embedded,output_shape,patch_size):
    ...
    # median_at =median_at.long()
    '''
    I1,I2 = torch.meshgrid(torch.arange(h,device=device),torch.arange(w,device=device),indexing='ij')
    # I1,I2 = torch.meshgrid(torch.arange(h-2),torch.arange(w-2),indexing='ij')
    # out2 = torch.zeros(h,w,ph*pw)
    # out2[1:-1,1:-1] = Vs
    # # out2[I1,I2,median_at[0,0]]
