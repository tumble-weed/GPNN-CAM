...
from saliency import get_saliency,augment_using_I
...
def augmentation_attribution(config=None):
    ...
    # cam0 = torch.zeros_like(cam0);cam0[:,:,-100:,-100:] = 1; print('setting cam0 to lower right')
...
def main(config=None):
    ...
    model = gpnn(config)
    with Timer('model run'):
        augmentations,I = model.run(to_save=False)
    
    #==============================================
    #import pdb;pdb.set_trace()
    if False and 'identity I':
        print('setting I to recreate ref image')
        M = (augmentations.shape[-2] - 2*(config['patch_size']//2)) * (augmentations.shape[-1] -
        2*(config['patch_size']//2))
        I = torch.tile(torch.arange(M).to('cuda'),(augmentations.shape[0],))
        I = I[:,None]
        
    torch.cuda.empty_cache()
    # output_im = skimage.io.imread(output_imname)
    saliency_dict = get_saliency(augmentations,I,(config['patch_size'],config['patch_size']),imagenet_target,saliency_method='gradcam')
    """
    '''
    saliency_dict = dict(saliency=saliency,raw_saliency=raw_saliency,smoothed_saliency=smoothed_saliency,avg_saliency=avg_saliency,smoothed_mass=smoothed_mass,
    ...
    if True:
        ...
        for i in reversed(range(11)):
            save_graph(model.trends['D',i],'D',
                       os.path.join(config['out_dir'],f'D{i}.png'))
            save_graph(model.trends['change_in_I',i],'change_in_I',
                       os.path.join(config['out_dir'],f'change_in_I{i}.png'))
            save_graph(model.trends['diversity',i],'diversity',
                       os.path.join(config['out_dir'],f'diversity{i}.png'))
            save_graph(model.trends['change_in_image',i],'change_in_image',
                       os.path.join(config['out_dir'],f'change_in_image{i}.png'))
            # import pdb;pdb.set_trace()
        '''
    ...
    for ii,ci in enumerate(cams):
        img_save(ci[0], 'cam'+model.out_file[:-len('.png')] + str(ii) + '.png' )
    #==============================================
    #unpermuted cams
    ...
    for ii,ci in enumerate(unpermuted_cams):
        img_save(ci[0], 'unpermuted_cams'+model.out_file[:-len('.png')] + str(ii) + '.png' )
    #====================================================================
    # std
    ...
    if True:
        img_save(std_cam_[0,0], 'unpermuted_cams'+model.out_file[:-len('.png')] + 'std' + '.png' )

    avg_saliency_ = saliency_dict['avg_saliency']
    assert avg_saliency_.ndim == 4
    assert avg_saliency_.shape[1] == 1
    img_save(avg_saliency_[0,0], 'unpermuted_cams'+model.out_file[:-len('.png')] + 'avg_saliency' + '.png' )
    if False:
