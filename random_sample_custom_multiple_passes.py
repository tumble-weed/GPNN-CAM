...
from saliency import get_saliency,permute_using_I
...
def main(config=None):
    ...
    for i in range(n_super_iters):
        ...
        if i > 0:
            ...
            assert previous_saliency_dict['avg_saliency'].ndim == 4
            assert previous_saliency_dict['avg_saliency'].max() <= 1
            assert previous_saliency_dict['avg_saliency'].min() >= 0
            # running_saliency_dict['avg_saliency'] = running_saliency_dict
            total_mass = running_saliency_dict['mass_sum']  + previous_saliency_dict['mass_sum']
            # import ipdb;ipdb.set_trace()
            running_saliency_dict['avg_saliency'] =  (running_saliency_dict['avg_saliency']*running_saliency_dict['mass_sum'] + previous_saliency_dict['avg_saliency'] * previous_saliency_dict['mass_sum'])/(total_mass + (total_mass==0).astype(np.float32))
        ...
        if i>0:
            ...
            img_save(running_avg_saliency_[0,0], os.path.join(config_i['out_dir'],'unpermuted_cams', 'running_avg_saliency' + '.png' ))
